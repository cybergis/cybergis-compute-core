...
class SingularityConnector extends SlurmConnector {
    ...
    execExecutableManifestWithinImage(manifest: executableManifest, config: slurm) {
        ...
        if (manifest.pre_processing_stage) {
            ...
            cmd += `singularity exec ${this._getVolumeBindCMD()} ${containerPath} cd ${this.getContainerExecutableFolderPath()} && ${preProcessingStage}\n`
        }
        ...
        cmd += `srun --mpi=pmi2 singularity exec ${this._getVolumeBindCMD()} ${containerPath} cd ${this.getContainerExecutableFolderPath()} && ${executionStage}\n`
        ...
        if (manifest.post_processing_stage) {
            ...
            cmd += `singularity exec ${this._getVolumeBindCMD()} ${containerPath} cd ${this.getContainerExecutableFolderPath()} && ${postProcessingStage}`
        }
