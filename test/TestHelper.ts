...
export default class TestHelper {
    static async createJob(id: string, userId: string, secretToken: string, maintainer: string, hpc: string): Promise<Job> {
        const db = new DB()
    ...
    static async createEvent(job: Job, type: string, message: string): Promise<Event> {
        const db = new DB()
        const connection = await db.connect()
