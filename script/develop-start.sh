...
if [[ ! -z "${RUN_IN_BACKGROUND}" ]]; then
  docker-compose -f ./docker-compose.yml --log-level ERROR up -d --remove-orphans job_supervisor
else
  docker-compose -f ./docker-compose.yml --log-level ERROR up --remove-orphans job_supervisor
fi